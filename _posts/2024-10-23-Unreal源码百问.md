...
IsDebuggerPresent 是 Windows API 中的一个函数，主要用于检测当前运行的程序是否正在被调试。
如果程序被如 Visual Studio 这样的调试器附加时，此函数会返回 TRUE；否则，它会返回 FALSE。
...
### 15.Slate示例在哪里?
源码工程中设置SlateViewer为启动项即可，Engine/Source/Programs/SlateViewer/Private/SlateViewerApp.cpp
默认只显示一个示例窗口，启动加testsuite参数可显示更详尽的
```c
...
rem Extracting dependencies archive files
.\Engine\Binaries\DotNET\7z\7za.exe x .\Engine\Binaries\DotNET\GitDependencies\Dependencies.7z.001
...
+  解压Dependencies.7z.001
+  检测GitDependencies启动参数
+  启动\Engine\Binaries\DotNET\GitDependencies\win-x64\GitDependencies.exe
+  设置 Git 钩子
+  安装必备软件UEPrereqSetup_x64.exe
+  注册引擎安装UnrealVersionSelector-Win64-Shipping.exe

GitDependencies.exe的源码在Engine\Source\Programs\GitDependencies
启动http下载所有依赖资源，cache存在.git的ue-gitdeps目录下，下完后通过.uedependencies文件用于检测下载的资源是否完整，并重命名到正确的目录。


GitDependencies.exe参数如下
```csharp
...
call  %~dp0Engine\Build\BatchFiles\RunUAT.bat BuildGraph -target="Make Installed Build Win64" -Script="Engine/Build/InstalledEngineBuild.xml" -set:HostPlatformOnly=true -set:WithDDC=false -set:GameConfigurations=Development -set:SignExecutables=false
pause

```

+  步骤配置InstalledEngineBuild.xml
+  Strip配置InstalledEngineFilters.xml

占用空间：

引擎基础大小21.7GB，下完依赖和生成工程后99.7 GB，BuildGraph执行后446 GB，去掉plugins和UnrealEditor-*.pdb 30.5 GB 
...
UnrealInsights-*.pdb
UnrealFrontend-*.pdb
UnrealMultiUserSlateServer-*.pdb
UnrealLightmass-*.pdb
...
修改Engine/Build/InstalledEngineFilters.xml，过滤掉pdb
<!-- Content -->
Engine/Content/....psd
Engine/Content/....pdn
Engine/Content/....po
...
### 19.Unreal版本注册
- 引擎在创建项目时，会判断引擎是预编译版本(Binary Build)还是源码编译版本(Source Build)，判断的方法是检测引擎%Engine-Folder%\Engine\Build路径下的文件夹里是否有以下两个文件之一：
  - InstalledBuild.txt：代表是预编译版本；
  - SourceDistribution.txt：代表是源码编译版本。
  
如果是源码版本会加-engine参数

bool FDesktopPlatformBase::IsSource Distribution(corst FString&EngineRootDir)

预编译版引擎和源码编译版引擎在创建项目时，创建项目工程文件时有不同的操作，如果是源码编译版引擎，则会在项目工程中引入引擎的所有工程，通过修改上述文件可以避免编译引擎源码


- 注册Unreal版本的机制原理如下：
https://github.com/hxhb/UE4Launcher/blob/master/Source/Private/Tools/EngineLaunchTools.cpp

void FDesktopPlatformWindows::EnumerateEngineInstallations(TMap<FString, FString> &OutInstallations)

获取当前机器上的unreal，搜的是注册表HKEY_CURRENT_USER\Software\Epic Games\Unreal Engine\Builds

bool FDesktopPlatformWindows::RegisterEngineInstallation(const FString &RootDir, FString &OutIdentifier) 注册这个unreal版本

右键菜单注册表 HKEY_LOCAL_MACHINE\SOFTWARE\Classes\Unreal.ProjectiFile\shell\rungenproj

写右键菜单 FDesktopPlatformWindows::VerifyFileAssociations()

void FDesktopPlatformWindows::GetRequiredRegistrySettings(TIndirectArray<FRegistryRootedKey> &RootedKeys)


注册Unreal版本
UnrealVersionSelector-Win64-Shipping.exe必须改为UnrealVersionSelector.exe才能用，其作用是注册Unreal版本

Engine\Source\Programs\UnrealVersionSelector\Private\UnrealVersionSelector.cpp

UnrealVersionSelector.cpp if(!FDesktopPlatformModule::Get()->VerifyFileAssociations())


### 19.编译UnrealGameSync

编译脚本生成UnrealGameSync安装包
```bat
@echo off

call  %~dp0Engine\Build\BatchFiles\RunUAT.bat BuildGraph -target="Build UnrealGameSync Installer" -Script="Engine/Source/Programs/UnrealGameSync/BuildUGS.xml" 
pause
```

LaunchEditor时必须在编辑器目录下放个Editor\Engine.version文件用于识别编辑器