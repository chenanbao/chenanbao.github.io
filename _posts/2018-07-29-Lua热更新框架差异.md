...
[Lua Interface](http://lua-users.org/wiki/LuaInterface)是一个用于Lua语言和Microsoft .NET的公共语言运行时（CLR）之间集成的库,LuaInterface的作者后来开发出跨平台升级版本[NLua](https://github.com/NLua/NLua)。Tolua的前身Ulua就是基于Lua Interface开发的，包括后来的Slua、Xlua可以说都是Lua Interface思想的变种者。
...
早期一些Lua框架调用C#采用反射实现(因为Lua Interface就是反射实现),会有性能损耗(几个框架测评相互嘲讽的点)，目前大多数框架都是采用Wrap方式生成胶水代码,因此这个性能差异基本抹平了。
...
Slua新的版本做了改进，可以CustomLuaClass]标记导出对应delegate，调用luafunction.cast转化对应的delegate,以避免gc开销.
...
getMoney = getFunction.cast<GetMoneyDelegate>();
...
| 生成wrap| Unity和Custom分别生成、可视化设置生成输出目录，开发就要生成胶水代码。 | 拆分成wrap+delegates+binder，开发就要生成胶水代码。 | 开发时可以用反射,发布时再生成。Unity和Custom合并生成，可以自定义模板输出自定义格式 |
