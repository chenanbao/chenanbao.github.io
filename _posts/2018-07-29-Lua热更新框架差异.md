...
实际开发中大多是Lua调用C#，保持二个原则提高性能：1.减少对象访问，如上分析框架得知这个交互过程其实很长，代价很高。2.减少参数，尽量传valueType，映射好类型避免boxing。
...
//UIControlWrap.cs 这里设置对象属性Vector3，只是传递二个float。对象transform访问完全是在c#侧直接操作，很大程度提升性能。
