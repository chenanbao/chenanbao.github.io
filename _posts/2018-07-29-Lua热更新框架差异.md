...
早期一些Lua框架调用C#采用反射实现,会有性能损耗(几个框架测评相互嘲讽的点)，目前大多数框架都是采用Wrap方式生成胶水代码,因此这个性能差异基本抹平了。
...
4.复杂类型
slua只是对特定Unity类型,做了table映射处理。lua capi都在C侧做的批处理优化。
但是xlua.genaccessor做的更通用，也是在C侧完成操作，并且还提供Struct映射机制解决boxing问题。
...
| 生成wrap| Unity和Custom分别生成、可视化设置生成输出目录 | 拆分成wrap+delegates+binder | 开发时可以用反射,发布时再生成。Unity和Custom合并生成，可以自定义模板输出自定义格式 |
| Build Lua库 | 硬编码脚本Build、只有luajit源码、无Lua源码。C#集成了luajit烘焙lua工具 | [独立runtime工程](https://github.com/topameng/tolua_runtime) mingw+Msys2 build | 依赖CMake Build，集成lib很方便、luajit和lua最新几个版本源码都有 |
