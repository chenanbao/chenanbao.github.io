...
unity中的欧拉角有两种方式可以解释:
1,当认为顺序是yxz时（其实就是heading - pitch - bank），约定是让方位从惯性坐标系到最终的物体自身坐标系

![image](/img/u_104.png)

2，当认为顺序是zxy时（roll - pitch - yaw），约定是从最终的物体坐标系到最开始的惯性坐标系的旋转变换
...
$q = \left[\begin{matrix} 0  \\ sin(\theta /2)  \\ 0  \\  cos(\theta /2)  \end{matrix}\right]\left[\begin{matrix}  sin(\phi /2) \\ 0  \\ 0  \\ cos(\phi  /2)  
 \end{matrix}\right]\left[\begin{matrix}   0  \\ sin(\psi  /2)  \\ 0  \\ cos( \psi  /2)  \end{matrix}\right] = \left[\begin{matrix} 0  \\ sin(y/2)  \\ 0  \\  cos(y/2)  \end{matrix}\right]\left[\begin{matrix}  sin(x/2) \\ 0  \\ 0  \\ cos(x  /2)  
 \end{matrix}\right]\left[\begin{matrix}   0  \\ sin(z  /2)  \\ 0  \\ cos( z  /2)  \end{matrix}\right]$
