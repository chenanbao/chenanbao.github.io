...
```
  ...
  矩阵T和点p（列矩阵）相乘记做Tp，**列矩阵从右向左读，读p右乘T。**

  ![image](/img/pos_84.png)

 p（行矩阵）和矩阵相乘记做pT。**行矩阵从左向右读，读p左乘T。**
...
##### 在左手系点绕z轴顺时针旋转的矩阵
 ...
 如果点P是绕z轴顺时针旋转β到P'，注意下图是绕z轴的负方向旋转的
...
矩阵计算如下图，对比上面右手坐标系绕z轴逆时针旋转矩阵，可看出差异：符号对角对调了。
...
下图为欧拉旋转公式，注意是基于右手坐标系的，旋转顺序是zxy，计算顺序从左向右。
...
**unity中的rotation就是一种外旋，静态的，是以惯性坐标系为轴的。Unity欧拉角使用zxy(roll - pitch - yaw)的顺规，Unity没有直接用欧拉角的生成矩阵。**
...
v旋转后的坐标v'为：$v'= qvq^{-1}$,当q为单位四元数则$v'= qvq*$

我们像得到一个矩阵M，有$v' = Mv$,假设$v=0+xi+yi+zk,q=w+qxi+qyj+qzk$
带入上式有：
$qvq^{-1} = (w+qxi+qyj+qzk)(0+xi+yi+zk)(w-qxi-qyj-qzk)$
$qvq^{-1} = (- qx*x -qy*y-qz*z + i (w*x + qy*z-qz*y)+ j (w*y - qx*z + qz*x)+ k(w*z + qx*y - qy*x))*(w - i qx - j qy - k qz)$
全部展开后如下式：
$qvq^{-1} = - qx*w*x -qy*w*y-qz*w*z + w*qx*x + qy*qx*z - qz*qx*y + w*qy*y - qx*qy*z + qz*qy*x + w*qz*z + qx*qz*y - qy*qz*x $
$+ i (w*w*x + qy*w*z-qz*w*y + qx*qx*x + qy*qx*y + qz*qx*z - w*qz*y + qx*qz*z - qz*qz*x + w*qy*z + qx*qy*y - qy*qy*x)$
$+ j (qx*qy*x + qy*qy*y + qz*qy*z + w*qz*x + qy*qz*z-qz*qz*y + w*w*y - qx*w*z + qz*w*x - w*qx*z - qx*qx*y + qy*qx*x)$
$+ k (qx*qz*x + qy*qz*y + qz*qz*z - w*qy*x - qy*qy*z + qz*qy*y + w*qx*y - qx*qx*z + qz*qx*x + w*w*z + qx*w*y - qy*w*x)$

将x，y和z项分组并将它们放在矩阵中给出：

| 列1 | 列2 | 列3 |
| ------ | ------ | ------ |
| $w^2 + qx^2-q^2-qy^2$ | $-qz*w + qy*qx-w*qz + qx*qy$ | $qy*w + qz*qx + qx*qz + w*qy$ |
| $qx*qy + w*qz + qz*w + qy*qx$ |$qy^2-qz^2 + w^2 - qx^2$ | $qz*qy + qy*qz-qx*w-w*qx$ |
| $qx*qz-w*qy + qz*qx-qy*w$ | $qy*qz + qz*qy + w*qx + qx*w$ | $qz^2-qy^2-qx^2 + w^2$ |

如果$w^2 + qx^2 + qz^2 + qy^2 = 1$

| 列1 | 列2 | 列3 |
| ------ | ------ | ------ |
|1-2(qz^2 + qy^2)|2(qy*qx - qz*w)|2(qy*w + qz*qx)|
|2(qx*qy + w*qz)|1-2(qz^2 + qx^2)|2(qz*qy - qx*w)|
|2(qx*qz - w*qy)|2(qy*qz + w*qx)|1-2(qy^2 + qx^2)|
