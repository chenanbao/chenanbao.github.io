...
四元数和它的共轭q*代表相反的角位移，因为相当于旋转轴反向。四元数的逆$q^{-1}$。

![image](/img/u_90.png)

如果$\vert q \vert  = 1$，则$q^{-1} = q*$，所以$q^{-1}$和$q*$是相同的旋转。
...
$= \vec{v}_\bot \cos \alpha + (\vec{u} \times \vec{v}) \sin \alpha + \vec{v}_{\|} $
