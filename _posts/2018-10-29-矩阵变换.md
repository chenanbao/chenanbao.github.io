...
定义二个四元数，分为标量a与向量v，$q_{1}=(w_{1},\vec v_{1})$,$q_{2}=(w_{2},\vec v_{2})$,  $\vec v_{1}$表示矢量$ (x_{1},y_{1},z_{1}),\vec v_{1} = (x_{1}i,y_{1}j,z_{1}k)$ ,$\vec v_{2}$表示矢量$ (x_{2},y_{2},z_{2}),\vec v_{2} = (x_{2}i,y_{2}j,z_{2}k)$。



跟复数、向量和矩阵一样，两个四元数之和需要将不同的元素加起来。

$q_{1}+q_{2} = (w_{1}+w_{2},\vec v_{1}+\vec v_{2})$

两个四元数之间的非可换乘积通常被格拉斯曼（Hermann Grassmann）称为积

$q_{1}q_{2} = (w_{1}+x_{1}i+y_{1}j+z_{1}k)(w_{2}+x_{2}i+y_{2}j+z_{2}k) $

$q_{1}q_{2} = (w_{1}*w_{2} - (x_{1}*x_{2} + y_{1}*y_{2} + z_{1}*z_{2})) + (w_{1}*x_{2} + x_{1}*w_{2} + y_{1}*z_{2} - z_{1}*y_{2}) i + (w_{1}*y_{2} + y_{1}*w_{2} - x_{1}*z_{2} + z_{1}*x_{2}) j + (w_{1}*z_{2} + z_{1}*w_{2} + x_{1}*y_{2} - y_{1}*x_{2} ) k $


$q_{1}q_{2} = (w_{1}w_{2}-\vec v_{1}·\vec v_{2},w_{1}\vec v_{2}+w_{2}\vec v_{1}+\vec v_{1}×\vec v_{2})$

$q_{2}q_{1} =(w_{2}w_{1}-\vec v_{2}·\vec v_{1},w_{2}\vec v_{1}+w_{1}\vec v_{2}+\vec v_{2}×\vec v_{1})$
...
$\vec v_{1} × \vec v_{2} = (y_{1}z_{2}-z_{1}y_{2})i-(x_{1}z_{2}-z_{1}x_{2})j+(x_{1}y_{2}-y_{1}x_{2})k$

**四元数的模**

$ {\vert q \vert} = {\vert w\quad\vec v \vert} = {\vert w\quad(x,y,z) \vert}  = \sqrt{w^{2}+{\vec v}^2} = \sqrt{w^{2}+x^2+y^2+z^2} $
...
$ q* = \left[{w\quad\vec v }\right]* = \left[{w\quad(x,y,z)}\right]* =  \left[{ w\quad(-x,-y,-z) }\right]$

求四元数的逆$q^{-1}$

$ qq* = [w^{2}+{\vec v}^{2},w\vec v-w\vec v+\vec v×\vec v] = [w^{2}+{\vec v}^{2},0] =[{\vert q \vert}^{2},0]$
