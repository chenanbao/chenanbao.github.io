...
**在unity中transform.Rotate(Vector3 eularAngles Space relativeTo) 如果relativeTo为Space.Self，采用yxz顺规。**

当认为顺序是yxz时（其实就是heading - pitch - bank），规定使用左手坐标系，+x向右，+y向上，+z向前。

a. 此时物体坐标系和惯性坐标系重合，heading为绕y轴的旋转量，向右旋转为正（如果从上面看，旋转正方向就是顺时针方向），
b. 经过heading旋转后，pitch为绕x轴的旋转量，注意是物体坐标系的x轴，不是原惯性坐标系的x轴。依然遵守左手法则，向下旋转为正
c. 最后，经过了heading和pitch，bank为绕z轴的旋转量。再次提醒，是物体坐标系的z轴，不是原惯性坐标系的z轴。依据左手法则，从原点向+z看，逆时针方向为正
...
**Unity欧拉角使用zxy(roll - pitch - yaw)的顺规，它一种外旋，静态的，是以惯性坐标系为轴的。**

![image](/img/u_62.jpg)

Unity(Transform.eulerAngles)约定zxy顺序旋转 ：
...
下面旋转矩阵使用泰特布莱恩角(Tait–Bryan angles)的yxz顺规.
