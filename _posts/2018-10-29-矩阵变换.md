...
v旋转后的坐标v'为：$v'= qvq^{-1}$,当q为单位四元数则v'= qvq*

我们像得到一个矩阵M，有v' = Mv,假设v=0+xi+yi+zk,q=w+qxi+qyj+qzk，带入式有：

qvq* = (w+qxi+qyj+qzk)(0+xi+yi+zk)(w-qxi-qyj-qzk)

qvq* = (- qx*x -qy*y-qz*z + i (w*x + qy*z-qz*y)+ j (w*y - qx*z + qz*x)+ k(w*z + qx*y - qy*x))*(w - i qx - j qy - k qz)

全部展开后如下式：

qvq* = - qx*w*x -qy*w*y-qz*w*z + w*qx*x + qy*qx*z - qz*qx*y + w*qy*y - qx*qy*z + qz*qy*x + w*qz*z + qx*qz*y - qy*qz*x 
