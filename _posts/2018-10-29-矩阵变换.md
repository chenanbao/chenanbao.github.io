...
矩阵计算如下图，对比上面右手坐标系绕z轴逆时针旋转矩阵，实际是同一个矩阵，只是这里用的行向量左乘的旋转矩阵。

$\begin{bmatrix}
\color{Red}x' & \color{Red}y' & \color{Red}z' & \color{Red}1\\
\end{bmatrix}=\begin{bmatrix}
\color{Red}x & \color{Red}y & \color{Red}z & \color{Red}1\\
\end{bmatrix}R_{z}(\theta_{z})
 =\begin{bmatrix}
\color{Red}x & \color{Red}y & \color{Red}z & \color{Red}1\\
\end{bmatrix}\begin{bmatrix}
cos\theta & sin\theta & \color{Blue}0 & 0 \\ 
-sin\theta & cos\theta & \color{Blue}0 &0 \\ 
\color{Blue}0 & \color{Blue}0 & \color{Blue}1 & \color{Blue}0 \\ 
0 & 0 & \color{Blue}0 & 1
\end{bmatrix} = \begin{bmatrix}
\color{Red}xcos\theta-ysin\theta & \color{Red}xsin\theta+ycos\theta & \color{Red}z & \color{Red}1
\end{bmatrix}
      $
