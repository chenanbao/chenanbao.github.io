...
多边形偏移是一个挺有用的技巧，有时会用来解决[Z-fighting](https://en.wikipedia.org/wiki/Z-fighting)。z-fighting在开启深度测试时，如果两个重叠物体的深度值非常接近，两次绘制到同一个像素时，深度值相差非常小，就有可能无法正确的辨别究竟哪一次绘制的深度值更小.那么就有可能会产生z-fighting.
...
| Geometry| 	2000	| 这是默认的渲染队列。它被用于绝大多数对象。不透明几何体使用该队列。**从前往后渲染**| 
