...
正因为**marshall是一个很重量的工作**，所以考量一个Lua热更新框架是否优良的标准之一就是LuaL指令是否合理调用和数据传递的处理
...
AOT编译器(il2cpp.exe)将由Mono输出的中间语言(IL)代码生成为C++代码,这种实现使得可以提高约一到二倍性能,最重要是可以抹平一些p/invoke的消耗。早期IL2CPP二个使用场景是iOS 64-bit和WebGL,后来Android也支持了IL2CPP.
