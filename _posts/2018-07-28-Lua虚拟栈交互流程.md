...
在托管代码c#层面，所有的数据类型都被分为两类：[blittable或者non-blittable](https://docs.microsoft.com/en-us/dotnet/framework/interop/blittable-and-non-blittable-types)。

**blittable类型**意味着在托管(managed)代码和非托管代码中，内存的结构是一致的，没有区别（如：system.byte，system.int16，system.intptr），blittable类型数据能够直接传递给原生代码。

**Non-blittable类型**在两者中的内存表现就不一致。（如：system.boolean，system.string，system.array）。non-blittable类型就需要做转换工作了。而这个转换工作很自然的就牵扯到新内存的分配。

正因为**[marshaling](https://msdn.microsoft.com/zh-cn/library/system.runtime.interopservices.marshal(v=vs.110).aspx)是一个很重量的工作**，所以考量一个Lua热更新框架是否优良的标准之一就是LuaL指令是否合理调用和数据传递的处理
