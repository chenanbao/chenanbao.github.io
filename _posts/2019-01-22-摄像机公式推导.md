...
观察坐标中的x被映射到x'，其通过使用相似三角形的比率来计算；如上正交投影x’和y‘映射到具有线性关系的NDC的x_ndc和y_ndc。

$\frac{x'-l}{r-l}=\frac{x_{ndc}-(-1)}{1-(-1)} \Rightarrow  x_{ndc}=\frac{2}{r-l}x'-\frac{r+l}{r-l}$

$\frac{y'-b}{t-b}=\frac{y_{ndc}-(-1)}{1-(-1)} \Rightarrow  y_{ndc}=\frac{2}{t-b}y'-\frac{t+b}{t-b}$
